...
const getdata = async (Mu_place, Mu_place_lat, Mu_place_long) => { //krong
    try {
        let res = await fetch(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${Mu_place_lat},${Mu_place_long}&radius=20000&keyword=โรงแรม&key=AIzaSyBs3XYHMlCfX7P3vJFrKK_xsiTyztOaQzc&language=th`);
...
const savee = async () => {
    try {
        ...
        fs.writeFileSync('HotelData.json', data);
    } catch (error) {
        console.error( error);
    }
};
