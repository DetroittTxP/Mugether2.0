...
const getdata = async (Mu_place, Mu_place_lat, Mu_place_long) => {
    try {
        let res = await fetch(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${Mu_place_lat},${Mu_place_long}&radius=20000&keyword=ที่เที่ยวใกล้ฉัน&key=AIzaSyDJ97OVzMevOzoFgXZ2byUfhwEcJuT02JE&language=th`);
...
const saveDataToFile = async () => {
    try {
        ...
        fs.writeFileSync('Travel.json', data);
        console.log("saved");
    } catch (error) {
        console.error( error);
    }
