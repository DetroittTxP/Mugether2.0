...
const getrawdata = async (Mu_place, Mu_place_lat, Mu_place_long) => {
    try{
        let res = await fetch(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${Mu_place_lat},${Mu_place_long}&radius=15000&keyword=โรงเเรม&key=AIzaSyCAA86m3sjNW8C1mCXvHtk2_He59BWytCI&language=th`)
...
const save = async () =>{
    try{
        ...
        fs.writeFileSync('Hotel_mu.json',JSON.stringify(data,null,2));
        console.log('saved');
        return;
    }
    catch(err){
